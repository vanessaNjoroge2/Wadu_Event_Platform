    ...
    <Layout>
      <div className="min-h-[85vh] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-8 space-y-7">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  {selectedRole === "attendee" && (
                    <CheckCircle2 size={16} className="absolute top-2.5 right-2.5 text-purple-400" />
                  )}
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition ${
                    selectedRole === "attendee" ? "bg-purple-600" : "bg-slate-700"
                  }`}>
                    <User size={20} className="text-white" />
                  </div>
                  <div className="text-center">
                    <p className="text-white font-bold text-sm">Attendee</p>
                    <p className="text-gray-400 text-xs mt-0.5">Buy tickets & explore events</p>
                  </div>
                </button>
                ...
                <button
                  ...
                  className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${
                    selectedRole === "organizer"
                      ? "border-[#00C2A8] bg-[#00C2A8]/10 shadow-lg shadow-[#00C2A8]/20"
                      : "border-slate-600 bg-slate-900/50 hover:border-slate-500"
                  }`}
                >
                  {selectedRole === "organizer" && (
                    <CheckCircle2 size={16} className="absolute top-2.5 right-2.5 text-[#00C2A8]" />
                  )}
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition ${
                    selectedRole === "organizer" ? "bg-[#00C2A8]" : "bg-slate-700"
                  }`}>
                    <Building2 size={20} className="text-white" />
                  </div>
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <p className="text-white font-bold text-sm">Organizer</p>
                      <span className="text-[10px] bg-[#00C2A8]/20 text-[#00C2A8] border border-[#00C2A8]/40 px-1.5 py-px rounded-full font-semibold">Pro</span>
                    </div>
                    <p className="text-gray-400 text-xs mt-0.5">Manage events & payouts</p>
                  </div>
                </button>
              </div>
            </div>
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    {tab === "signin" && (
                      <Link to="#" className="text-purple-400 text-xs hover:text-purple-300">Forgot password?</Link>
                    )}
                  </div>
                  <div className="relative">
                ...
                {/* CTA button — colour adapts to selected role */}
                <button
                  type="submit"
                  className={`w-full text-white py-3.5 rounded-xl font-bold transition flex items-center justify-center gap-2 shadow-lg ${
                    selectedRole === "organizer"
                      ? "bg-gradient-to-r from-[#00C2A8] to-[#0097A7] hover:from-[#00b09a] hover:to-[#008899] shadow-[#00C2A8]/30"
